...
int insert_symbol(char *name, unsigned kind, unsigned type, 
  ...
  symbol_table[index].atr2[atr1] = atr2;
  symbol_table[index].vr = vrijednost;
  return index;
}
...
int insert_literal(char *str, unsigned type) {
  ...
  idx = insert_symbol(str, LIT, type, NO_ATR, NO_ATR, NO_VAL);
  return idx;
}

void set_check(int idx, int ch){
	symbol_table[idx].atr1 = ch;
}
...
void set_atr2(int index, unsigned atr2, int ip) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
     symbol_table[index].atr2[ip] = atr2;
}

unsigned get_atr2(int index, int ip) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].atr2[ip];
  return NO_ATR;
}
...
void print_symtab(void) {
  static const char *symbol_kinds[] = { 
    "NONE", "REG", "LIT", "FUN", "VAR", "PAR", "GLOB" };
  int i,j;
  printf("\n\nSYMBOL TABLE\n");
  printf("\n       name           kind   type  vr  atr1   atr2");
  printf("\n-- ---------------- -------- ----  --- -----  -----");
  for(i = 0; i < first_empty; i++) {
    printf("\n%2d %-19s %-4s %4d %4d  %4d  %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d", i, 
    symbol_table[i].name, 
...
void init_symtab(void) {
  ...
  for(i = 0; i < 14; i++) {
    sprintf(s, "%%%d", i);
    insert_symbol(strdup(s), REG, NO_TYPE, NO_ATR, NO_ATR, NO_VAL);
  }
}
